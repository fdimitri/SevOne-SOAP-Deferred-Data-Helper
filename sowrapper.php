<?php
/* Sample usage:
$soTest = new soDeferredDeviceWrapper("pm99.srv", "/soap3/api.wsdl", "username", "password", 0, 0);
...
$soTest->setDeviceName("accsw701.vod.prnynj.cv.net");
...
$soWrapperVersion = array('major' => 3, 'minor' => 0, 'revision' => 9, 'patch' => array(NULL), 'epoch' => 1407961392);
...
class soDeferredDeviceWrapper {
        ...
        protected function connectPeer() {
                ...
                if ($this->options['peer_mode']) {
                        ...
                        $newServerBase = "http://{$cPeer->name}.srv.hcvlny.cv.net";
                        $newServer = $newServerBase . "/soap3/api.wsdl";
                        $this->logMsg(SODW_INFO, "Connecting to {$newServer} (poller for {$this->deviceName})");
                        $vclient = $this->soCreateConnection($newServer, $options, "{$newServerBase}/soap3/api.php");
                        if (!$vclient) {
        ...
        protected function logMsg($logLevel, $sMsg) {
                ...
                $tag .= ")";
                if ($logLevel & $this->logLevel) echo "{$tag}\t{$fCall}():\t{$sMsg}\n";
        }
        ...
        public function attachIndicatorsToObject($objectName, $objectDesc, $objectTypeName, $indicator, $allowSumming = FALSE) {
                ...
                if (isset($indicator['name'])) {
                        $this->logMsg(SODW_WARN, "Indicator variable passed to us was not an array but was probably a regular indicator -- use attachIndicatorToObject instead");
                ...
                foreach ($indicator as $i) {
                        if (!isset($i['name']) || !isset($i['value'])) {
                                $this->logMsg(SODW_WARN, "This indicator does not appear to be valid -- name or value is unset: " . print_r($i, TRUE));
                                continue;
                        }
        ...
        public function attachIndicatorToObject($objectName, $objectDesc, $objectTypeName, $indicator, $allowSumming = FALSE ) {
                ...
                foreach ($this->intObjects[$this->deviceId][$objectName]['indicators'] as $i) {
                        if (!strcmp($indicator['name'], $i['name'])){
                                if ($allowSumming) {
                                ...
                                else $this->logMsg(SODW_WARN, "Asked to add {$objectName}->{$i['name']}, but indicator exists and not called with allowSumming");
                                return(FALSE);
                        }
                }
                $this->logMsg(SODW_INFO, "Adding {$indicator['name']} to {$objectName}");
        ...
        public function soInit($deviceName = NULL) {
                ...
                foreach ($servers as $server) {
                        $soapUrl = "http://{$server}{$url}";
                        $options = array('trace' => 1, 'connection_timeout'=>45, 'cache_wsdl'=>'1');
                        $client = $this->soCreateConnection($soapUrl, $options, "http://{$server}/soap3/api.php");

                        if (!$client) {
                                $this->logMsg(SODW_ERROR, "Could not connect to SOAP server at {$soapUrl}. Trying next server in list");
                                continue;
                        }
                        $this->client = $client;
        ...
        public function soCreateConnection($server, $options, $location = NULL) {
                $this->logMsg(SODW_FENTRY, "Called with " . print_r($server, TRUE) . " " . print_r($options, TRUE) . " " . print_r($location, TRUE));
                $client = new SoapClient($server, $options);
                if (!$client) {
